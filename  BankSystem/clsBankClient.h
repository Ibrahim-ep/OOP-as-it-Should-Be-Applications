...
private:
    ...
    string _PrepareTransferLogRecord(clsBankClient DestinationClient,float Amount, string UserName,string Seperator = "#//#")
    {
        ...
        sUser += _AccountNumber + Seperator;
        sUser += DestinationClient.AccountNumber() + Seperator;
        sUser += to_string(Amount) + Seperator;
        sUser += to_string(_AccountBalance) + Seperator;
        sUser += to_string(DestinationClient.AccountBalance()) + Seperator;
        sUser += UserName;
...
public:
    ...
    static stTransferLogRecord _ConvertTransferLogLineToRecord(string Line, string Seperator = "#//#")
    {
        ...
        TransferLogRecord.Date = vTransferData[0];
        TransferLogRecord.SourceAccountNumber = vTransferData[1];
        TransferLogRecord.DestinationAccountNumber = vTransferData[2];
        TransferLogRecord.Amount = stod(vTransferData[3]);
        TransferLogRecord.SourceBalance = stod(vTransferData[4]);
        TransferLogRecord.DestinationBalance = stod(vTransferData[5]);
        TransferLogRecord.Username = vTransferData[6];
    ...
    {
        vector <stTransferLogRecord> vLoginRegisterRecord;
        ...
        if (MyFile.is_open())
        {
            ...
            while (getline(MyFile, Line))
            {
                ...
                vLoginRegisterRecord.push_back(client);
            }
